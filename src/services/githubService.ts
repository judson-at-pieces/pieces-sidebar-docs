interface PullRequestOptions {
  ...
  useExistingBranch?: boolean; // New flag to use existing branch instead of creating new one
}
...
class GitHubService {
  ...
  async createPullRequest(
    ...
    try {
      const { title, body, files, baseBranch, headBranch, useExistingBranch } = options;
      
      // Generate head branch name if not provided and not using existing
      ...
      console.log('Using existing branch:', useExistingBranch);

      if (useExistingBranch && headBranch) {
        // Use existing branch - just update files and create PR
        console.log('Using existing branch:', finalHeadBranch);
        
        // Step 1: Get the latest commit SHA from the head branch (the branch we're PRing FROM)
        const headBranchResponse = await fetch(
          `https://api.github.com/repos/${repoConfig.owner}/${repoConfig.repo}/git/ref/heads/${finalHeadBranch}`,
          {
            headers: {
        ...
        if (!headBranchResponse.ok) {
          const errorText = await headBranchResponse.text();
          throw new Error(`Failed to get head branch ${finalHeadBranch}: ${errorText}`);
        }

        const headBranchData = await headBranchResponse.json();
        const headCommitSha = headBranchData.object.sha;

        // Step 2: Get the tree from the head commit
        const headCommitResponse = await fetch(
          `https://api.github.com/repos/${repoConfig.owner}/${repoConfig.repo}/git/commits/${headCommitSha}`,
          {
            headers: {
              'Authorization': `Bearer ${accessToken}`,
              'Accept': 'application/vnd.github.v3+json',
        ...
        const headCommitData = await headCommitResponse.json();
        const headTreeSha = headCommitData.tree.sha;
        ...
        // Step 7: Create the pull request (head -> base)
        const createPrResponse = await fetch(
          `https://api.github.com/repos/${repoConfig.owner}/${repoConfig.repo}/pulls`,
          {
            ...
            body: JSON.stringify({
              title,
              body,
              head: finalHeadBranch, // The existing branch with changes
              base: baseBranch,      // The target branch (main)
            }),
          }
        );
        ...
        // Original logic for creating new temporary branch
