...
export const createComponentMappings = () => ({
  ...
  tbody: ({ children, ...props }: any) => <tbody className="divide-y divide-slate-200 dark:divide-slate-800/80" {...props}>{children}</tbody>,
  ...
  Card: ({ title, image, children, ...props }: any) => {
    console.log('🎯 Component mapping Card rendering:', { title, image, hasChildren: !!children });
    return <Card title={title} image={image} {...props}>{children}</Card>;
  },
  CardGroup: ({ cols, children, ...props }: any) => {
    console.log('🎯 Component mapping CardGroup rendering:', { cols, childrenCount: React.Children.count(children) });
    return <CardGroup cols={cols} {...props}>{children}</CardGroup>;
  },
});

export type ComponentMappings = ReturnType<typeof createComponentMappings>;
