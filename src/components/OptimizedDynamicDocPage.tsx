...
import { OptimizedContentLoader } from './OptimizedContentLoader';

const OptimizedDynamicDocPage = memo(() => {
  ...
  // Memoize path variations to prevent recalculation
  ...
  useEffect(() => {
    async function loadContent() {
      if (!pathVariations.length) return;
  ...
  const isGettingStartedPage = useMemo(() => path === 'getting-started', [path]);
  const shouldShowTOC = useMemo(() => 
    !isGettingStartedPage && content?.content.includes('#'), 
    [isGettingStartedPage, content?.content]
  );
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 min-w-0 order-2 lg:order-1">
        ...
        {/* Optimized Content Loader */}
        <OptimizedContentLoader content={content} isGettingStartedPage={isGettingStartedPage} />
      </div>
...
OptimizedDynamicDocPage.displayName = 'OptimizedDynamicDocPage';

export { OptimizedDynamicDocPage };
